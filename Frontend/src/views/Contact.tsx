    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="bg-black text-white py-14 md:py-20">
        <RevealOnScroll className="container">
          <div className="text-xs uppercase tracking-[0.25em] text-white">Contact</div>
          <h1 className="font-serif text-4xl md:text-7xl mt-3 text-white">Let&apos;s talk</h1>
          <p className="mt-4 text-white/85 max-w-xl">
            Whether you&apos;re buying, listing, or just curious — our team is here to help.
          </p>
        </RevealOnScroll>
      </section>
      ...
      <section className="relative overflow-hidden py-10 md:py-16">
        ...
        <div className="container relative z-10 grid lg:grid-cols-[0.95fr_1.6fr] gap-6 md:gap-8">
            ...
            {contactCards.map((c) => (
              <div key={c.key} className="rounded-2xl border border-gold/25 bg-card p-5 md:p-6 shadow-soft">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-xs uppercase tracking-[0.22em] text-gold font-semibold">{c.label}</div>
                    <div className="mt-1 text-sm sm:text-base md:text-base text-foreground/90">{c.detail}</div>
                  </div>
                </div>
              </div>
            ))}
          ...
          <RevealOnScroll delayMs={60}>
            <form onSubmit={submit} className="rounded-2xl border border-gold/25 bg-card p-6 md:px-10 md:pt-8 md:pb-6 shadow-soft">
              <h2 className="font-serif text-3xl md:text-5xl">Send us a message</h2>
              ...
              <div className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div><Label>Email</Label><Input name="email" type="email" required placeholder="Your email" /></div>
                </div>
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <Label>Phone</Label>
          ...
          <div className="rounded-2xl overflow-hidden border border-border h-[280px] md:h-[400px] bg-muted relative">
            <div className="absolute inset-0 grid place-items-center">
              <div className="text-center">
                <MapPin className="h-14 w-14 mx-auto text-gold animate-float mb-2" />
                <p className="font-serif text-xl">Silver City Complex, Merikunnu, Kozhikode</p>
