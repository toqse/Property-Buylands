...
import { useStates, useDistricts, usePropertyTypes, useFeatures } from "@/hooks/api/useCatalog";
import { MAX_PROPERTY_IMAGES } from "@/lib/api/propertyForm";
...
const AREA_UNIT_OPTIONS: {
  ...
  { value: "sqft", label: "Square feet (built property)", placeholder: "e.g. 3200", inputLabel: "Area (sq.ft)" },
  { value: "cents", label: "Cent (plot/land)", placeholder: "e.g. 12", inputLabel: "Area (Cents)" },
];
...
const formCardClass = "rounded-2xl border border-border bg-card p-6 shadow-sm space-y-4";
...
function sanitizeAreaValue(raw: string, maxLen = 14): string {
...
/** Construction year: digits only, max 4 (submit validates range). */
function sanitizeBuiltYear(raw: string): string {
...
/** Map backend ownership codes to the form's Select option values. */
function normalizeOwnership(raw: string | undefined): string {
  const v = (raw ?? "").trim().toLowerCase();
  if (!v) return "";
  if (v === "direct_owner" || v === "owner") return "Owner";
...
function parkingSpacesFromFeatures(features: string[] | undefined, stored: string | undefined): string {
...
export function validateAndParseDraft(draft: AddPropertyDraft): { ok: false; message: string; field?: string } | { ok: true; data: DraftParseOk } {
  if (!draft.title.trim()) return { ok: false, message: "Please add a title for your property", field: "title" };
  if (!draft.stateId) return { ok: false, message: "Please select a state", field: "state" };
  if (!draft.districtId) return { ok: false, message: "Please select a district", field: "district" };
  if (!draft.city.trim()) return { ok: false, message: "Please enter a city", field: "city" };
  const latNum = Number.parseFloat(draft.latitude);
  const lngNum = Number.parseFloat(draft.longitude);
  if (!Number.isFinite(latNum) || !Number.isFinite(lngNum)) {
    return { ok: false, message: "Please enter valid latitude and longitude", field: "latitude" };
  }
  if (!draft.googleEmbedHtml.trim()) return { ok: false, message: "Google embedded map iframe code is required", field: "google_embedded_map_link" };
  if (!isValidEmail(draft.contactEmail)) {
    return { ok: false, message: "Enter a valid email address (no spaces or stray symbols)", field: "email" };
  }
  ...
  if (draft.phone.trim()) {
    if (phoneDigits.length < PHONE_DIGITS_MIN || phoneDigits.length > PHONE_DIGITS_MAX) {
      return { ok: false, message: `Phone number must be ${PHONE_DIGITS_MIN}–${PHONE_DIGITS_MAX} digits`, field: "phone_number" };
    }
  }
  if (draft.whatsapp.trim()) {
    if (whatsDigits.length < PHONE_DIGITS_MIN || whatsDigits.length > PHONE_DIGITS_MAX) {
      return { ok: false, message: `WhatsApp number must be ${PHONE_DIGITS_MIN}–${PHONE_DIGITS_MAX} digits`, field: "whatsapp_number" };
    }
  }
  const priceNum = parseMoneyValue(draft.price);
  if (priceNum === null || priceNum <= 0) return { ok: false, message: "Price must be a positive number (digits only)", field: "price" };
  let areaSq = 0;
  if (draft.area.trim()) {
    const parsedArea = parseMoneyValue(draft.area);
    if (parsedArea === null || parsedArea <= 0) {
      return { ok: false, message: "Area must be a positive number", field: "area" };
    }
    areaSq = parsedArea;
  }
  ...
  if (by) {
    ...
    if (by.length !== 4) return { ok: false, message: "Built year must be exactly 4 digits", field: "built_year" };
    const y = Number(by);
    if (!Number.isFinite(y) || y < 1800 || y > maxYear) {
      return { ok: false, message: `Built year must be from 1800 to ${maxYear}`, field: "built_year" };
    }
  }
  const feats: string[] = [];
  if (draft.parkingSpaces.trim()) feats.push(`${draft.parkingSpaces.trim()} parking space(s)`);
  const locationLine = [draft.city, draft.district, draft.state].filter(Boolean).join(", ") || "—";
  const nearbyFiltered = draft.nearbyPlaces.filter((n) => n.name.trim() || n.distanceKm.trim());
...
export function buildPropertyFromValidatedDraft(
  ...
  if (ctx.imageFiles.length > 0) {
    ...
    gallery = ctx.existing.gallery.length ? [...ctx.existing.gallery] : [ctx.existing.image];
  } else {
    gallery = [];
  ...
  else if (ctx.mode === "edit" && ctx.existing?.videoUrl) videoUrl = ctx.existing.videoUrl;
  ...
  const base: Property = {
    ...
    lng: parsed.lngNum,
    createdAt:
      ctx.mode === "edit" && ctx.existing ? ctx.existing.createdAt : new Date().toISOString().slice(0, 10),
    ...
    nearbyPlaces: parsed.nearbyFiltered.length ? parsed.nearbyFiltered : undefined,
  };

  if (ctx.mode === "edit" && ctx.existing) {
    if (ctx.existing.featured !== undefined) base.featured = ctx.existing.featured;
    if (ctx.existing.priceUnit !== undefined) base.priceUnit = ctx.existing.priceUnit;
  }
...
export function ListingFormFields({
  ...
  useEffect(() => {
    ...
    if (propertyTypesData.results.some((t) => t.name === draft.propertyCategory)) return;
    setDraft((d) => ({
      ...d,
      propertyCategory: propertyTypesData.results[0].name as Property["category"],
    }));
    ...
    <>
        ...
        <div className={cn("grid gap-4", !hideOwnership && "sm:grid-cols-2")}>
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) => setDraft((d) => ({ ...d, propertyFor: v as Property["type"] }))}
            >
      ...
      <div className={formCardClass}>
        ...
        <div className="grid sm:grid-cols-2 gap-4 pt-2">
          <div className="space-y-2 min-w-0" data-field="latitude">
            <Label>
            ...
            <Input
              ...
              onChange={(e) =>
                setDraft((d) => ({
                  ...d,
                  latitude: sanitizeCoordinate(e.target.value),
                }))}
            />
          </div>
          <div className="space-y-2 min-w-0" data-field="longitude">
            <Label>
            ...
            <Input
              ...
              onChange={(e) =>
                setDraft((d) => ({
                  ...d,
                  longitude: sanitizeCoordinate(e.target.value),
                }))}
            />
          </div>
        </div>
      ...
      <div className={formCardClass}>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          {draft.areaUnit === "cents" && (
            <p className="sm:col-span-2 -mt-1 text-[13px] text-muted-foreground">
              Bedrooms, bathrooms, built year, and furnishing are optional for cent listings.
            </p>
          )}
          <div className="space-y-2 min-w-0" data-field="area">
            <Label>
              {AREA_UNIT_OPTIONS.find((o) => o.value === draft.areaUnit)?.inputLabel ?? "Area"}{" "}
              <span className="text-red-500">*</span>
            </Label>
            <Input
              className="w-full min-w-0 max-w-full font-mono"
              placeholder={
                AREA_UNIT_OPTIONS.find((o) => o.value === draft.areaUnit)?.placeholder ?? "e.g. 3200"
              }
              inputMode={draft.areaUnit === "cents" ? "decimal" : "numeric"}
              value={draft.area}
              onChange={(e) =>
                setDraft((d) => ({
                  ...d,
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => setDraft((d) => ({ ...d, youtubeLink: e.target.value }))}
            />
          </div>
          {draft.areaUnit !== "cents" && (
            <>
              <div className="space-y-2" data-field="built_year">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDraft((d) => ({
                      ...d,
                      builtYear: sanitizeBuiltYear(e.target.value),
                    }))}
                />
              </div>
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setDraft((d) => ({ ...d, furnishing: v }))}
                >
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => setDraft((d) => ({ ...d, parkingSpaces: e.target.value }))}
            />
          </div>
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => setDraft((d) => ({ ...d, googleMapUrl: e.target.value }))}
          />
        </div>
        <div className="space-y-2" data-field="google_embedded_map_link">
          <Label>
            Google Embedded Map Link <span className="text-destructive">*</span>
          </Label>
          <Textarea
            rows={4}
            placeholder="Paste the Google Maps embed iframe code here…"
            value={draft.googleEmbedHtml}
            onChange={(e) => setDraft((d) => ({ ...d, googleEmbedHtml: e.target.value }))}
          />
        </div>
      </div>
      ...
      {/* Video file upload */}
      <div className={formCardClass}>
        <h3 className="font-semibold text-foreground">Property video (upload)</h3>
        <input
        ...
        <button
          ...
          onDrop={(e) => {
            e.preventDefault();
            e.stopPropagation();
            const f = Array.from(e.dataTransfer.files).find((file) => file.type.startsWith("video/"));
            if (f) setVideoFile(f);
          }}
          onClick={() => videoInputRef.current?.click()}
          className={cn(
            "w-full border-2 border-dashed rounded-xl p-8 text-center transition-colors hover:border-gold/60 cursor-pointer",
            videoFile ? "border-gold/40 bg-gold/5" : "border-border bg-muted/20",
          )}
        >
          <Upload className="h-8 w-8 mx-auto text-gold mb-2" />
          <p className="font-medium text-sm">
            {videoFile ? videoFile.name : "Drag & drop video or click to select"}
          </p>
        </button>
        {videoFile ? (
          <Button
        ...
        {existingVideoUrl && !videoFile ? (
          <div className="space-y-2">
            <p className="text-xs font-medium text-muted-foreground">Current video</p>
      ...
      {/* Nearby Places */}
      <div className={formCardClass}>
        <h3 className="font-semibold text-foreground">Nearby Places</h3>
        <div className="space-y-3">
          {draft.nearbyPlaces.map((row, idx) => (
            <div key={idx} className="flex flex-col sm:flex-row gap-2 sm:items-end">
              <div className="flex-1 space-y-2">
                <Label className="sr-only sm:not-sr-only">Place name</Label>
                <Input
                  placeholder="Place name (e.g., International Airport)"
                  value={row.name}
                  onChange={(e) => {
                    setDraft((d) => {
                      const next = [...d.nearbyPlaces];
                      next[idx] = { ...next[idx], name: e.target.value };
                      return { ...d, nearbyPlaces: next };
                    });
                  }}
                />
              </div>
              <div className="w-full sm:w-36 space-y-2">
                <Label className="sr-only sm:not-sr-only">Distance</Label>
                <Input
              ...
              <Button
                ...
                onClick={() =>
                  setDraft((d) => ({
                    ...d,
                    nearbyPlaces: d.nearbyPlaces.filter((_, i) => i !== idx),
                  }))}
              >
                Remove
              </Button>
          ...
          <Button
            ...
            onClick={() =>
              setDraft((d) => ({
                ...d,
                nearbyPlaces: [...d.nearbyPlaces, { name: "", distanceKm: "" }],
              }))}
          >
            Add Nearby Place
          </Button>
      ...
      <div className={formCardClass}>
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            {existingImages.length + imageFiles.length}/{MAX_PROPERTY_IMAGES} (at least 1 required)
          </span>
        </div>
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (list?.length) {
              setImageFiles((prev) => {
                const remaining = MAX_PROPERTY_IMAGES - existingImages.length - prev.length;
        ...
        {existingImages.length + imageFiles.length < MAX_PROPERTY_IMAGES ? (
          ...
          >
            ...
            <p className="font-medium text-sm text-foreground">Drag & drop files or click to select</p>
            <p className="text-xs text-muted-foreground mt-1">
              Up to {MAX_PROPERTY_IMAGES} images. You can add{" "}
              {MAX_PROPERTY_IMAGES - existingImages.length - imageFiles.length} more.
            </p>
          </div>
        ) : (
          <p className="text-xs text-muted-foreground rounded-xl border border-dashed border-border bg-muted/10 p-4 text-center">
            Maximum of {MAX_PROPERTY_IMAGES} images reached. Remove an image to add another.
          </p>
        )}
        {imageFiles.length > 0 ? (
